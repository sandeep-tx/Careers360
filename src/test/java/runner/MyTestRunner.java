...
@CucumberOptions(features = {"src/test/resources/features"}, glue = {"stepDefinitions", "hooks"}, plugin = {"pretty", "html:target/cucumber-html-report", "json:target/cucumber.json"},
        tags = "@RR002",
        monochrome = true
)
public class MyTestRunner extends AbstractTestNGCucumberTests {
    ...
    @AfterClass
    public void onFinish() throws XPathExpressionException, ParserConfigurationException, IOException, TransformerException, SAXException {
        LogUtil.infoLog(getClass(), " suite finished" + " at " + new Date());
        LogUtil.infoLog(getClass(), "\n\n+===========================================================================================================+");
        // extent.endTest(logger);

        if (System.getProperty("udid") == null) {
            if (System.getProperty("batch_type") == null) {
                extent.addSystemInfo("Execution Type", "ADHOC Testing");
//                KeywordUtil.extentReportHeading("ADHOC Testing Test Automation Report");
            } else if (System.getProperty("batch_type").equalsIgnoreCase("smoke")) {
                extent.addSystemInfo("Execution Type", "Android (Smoke)");
//                KeywordUtil.extentReportHeading("Android (Smoke) Test Automation Report");
            } else {
                extent.addSystemInfo("Execution Type", "Android (Regression)");
//                KeywordUtil.extentReportHeading("Android (Regression) Test Automation Report");
            }
        } else {
            if (System.getProperty("batch_type") == null)
                extent.addSystemInfo("Execution Type", "ADHOC Testing");
            else if (System.getProperty("batch_type").equalsIgnoreCase("smoke")) {
                extent.addSystemInfo("Execution Type", "IOS (Smoke)");
//                KeywordUtil.extentReportHeading("IOS (Smoke) Test Automation Report");
            } else {
                extent.addSystemInfo("Execution Type", "IOS (Regression)");
//                KeywordUtil.extentReportHeading("IOS (Regression) Test Automation Report");
            }
        }
        extent.flush();
        extent.close();
